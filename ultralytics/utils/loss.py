...
class v8CornersLoss(v8DetectionLoss):
    ...
    def __call__(self, preds, batch):
        ...
        # target_corners = target corners (batch_size, n, 28)

        loss = torch.zeros(31, device=self.device)  # box, cls, dfl, corner_loss (2*14)
        ...
        if fg_mask.sum():
            ...
            weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)
            loss[-28:] = torch.sum(torch.abs((gt_corners[fg_mask] - pred_corners[fg_mask]) * weight), dim=0)
        ...
        loss[1] *= self.hyp.cls  # cls gain
        loss[2] *= self.hyp.dfl  # dfl gain
        loss[-28:] *= self.hyp.closs  # corners gain
    ...
    def preprocess_corners(self, batch_idx, targets, batch_size):
        ...
        if targets.shape[0] == 0:
            ...
            counts = counts.to(dtype=torch.int32)
            out = torch.ones(batch_size, counts.max(), 28, device=self.device) * -1
            for j in range(batch_size):
                matches = batch_idx == j

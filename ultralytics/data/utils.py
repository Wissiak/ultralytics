...
def verify_image_label(args):
    ...
    if use_corners:
        corners = np.zeros((0, 14, 2), dtype=np.float32)
    try:
        # Verify images
        im = Image.open(im_file)
        im.verify()  # PIL verify
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint) and (not use_corners):  # is segment
            ...
            if nl:
                if keypoint:
                    ...
                    assert lb.shape[1] == 33, f"labels require 37 columns (cls,x,y,w,h,14*2 corner points), {lb.shape[1]} columns detected"
                    points = lb[:, 5:]
                    corners = points.reshape(-1, 14, 2) # (n, 14, 2) - 14 corners
